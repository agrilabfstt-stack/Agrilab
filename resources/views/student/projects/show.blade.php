        ...
        <div class="px-6 py-5">
            <div class="flex flex-col sm:flex-row sm:items-start gap-4 justify-between">
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-sm text-gray-400">Créé {{ $project->created_at->diffForHumans() }} · Dernière modification {{ $project->updated_at->diffForHumans() }}</p>
                </div>
                <div class="flex gap-2 flex-shrink-0">
                    @if(!$project->isBlocked())
                    <a href="{{ route('student.projects.edit', $project) }}"
                       class="inline-flex items-center gap-2 px-4 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-xl text-sm font-medium transition-colors">
                    ...
                    <form method="POST" action="{{ route('student.projects.destroy', $project) }}"
                        ...
                        @csrf @method('DELETE')
                        <button type="submit" class="inline-flex items-center gap-2 px-4 py-2.5 bg-red-50 hover:bg-red-100 text-red-600 rounded-xl text-sm font-medium transition-colors">
            ...
            @if($project->isBlocked())
            <div class="mt-4 bg-red-50 border border-red-200 rounded-xl p-3 flex items-center gap-2 text-sm text-red-700">
                <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                Ce projet est bloqué par votre professeur. Vous ne pouvez plus le modifier.
            </div>
            @endif
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6">
                ...
                @elseif($section->type === 'image')
                    ...
                    @else
                    <p class="text-gray-400 text-sm text-center py-4">Aucune image dans cette section.</p>
                    @endif
